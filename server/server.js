...
async function sendTransaction(res, contract, func, endoreser, assetId, owner) {
    try {
        if(func == 'getAsset'){
            console.log(`\n--> [${GRPC}] Evaluate Transaction: ReadAsset, function returns "asset1" attributes`);
            ...
            console.log(`\n--> [${GRPC}] Submit Transaction: TransferAsset asset${GRPC}, transfer to new owner`);
            await contract.submitTransaction('TransferAsset', `asset${GRPC}`, 'BBB');
