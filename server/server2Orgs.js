...
const { buildCCPOrgs, buildWallet } = require('../test-application/javascript/AppUtil.js');

const caHost = `ca.org${ORG}.example.com`;
const department = `org${ORG}.department1`;
const mspOrg = `Org${ORG}MSP`;
const channelName = 'mychannel';
const chaincodeName = 'basic';
const walletPath = path.join(__dirname, 'wallet');
const orgUserId = `appUser${ORG}${GRPC}`;

console.log(`Starting Fabric Server ${ORG}${GRPC}...\ncaHost:${caHost}\ndepartment:${department}\nmspOrg:${mspOrg}\n`)
...
async function getCC() {
	try {
		...
		const ccp = buildCCPOrgs(ORG.toString(), GRPC.toString());
...
async function transferAsset(res, contract, endoreser) {
    try {
        const assetId = `asset${ORG}${GRPC}${Math.floor(Math.random() * 1001).toString()}`;
        console.log(`\n--> [${ORG}${GRPC}] Submit Transaction: TransferAsset ${assetId}, transfer to new owner`);
...
async function createAsset(contract, assetId, color, size, owner, appraisedValue, endoreser) {
    try {
        console.log(`\n--> [${ORG}${GRPC}] Submit Transaction: CreateAsset ${assetId}`);
...
async function main(){
    ...
    console.log(`${ORG}${GRPC}: `,contract);

    for (let i = 1; i <= assetCount; i++) {
        await createAsset(contract, `asset${ORG}${GRPC}${i.toString()}`, 'red', `${i}`, 'Alice', `${100 + i}`);
    }
